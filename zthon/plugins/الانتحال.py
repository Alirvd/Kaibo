...
async def _(event):
    ...
    try:
        ...
        return await edit_delete(event, f"**نجب الانتحال مصار :**\n__{e}__")
    await event.client(functions.photos.UploadProfilePhotoRequest(pfile))
    await edit_delete(event, "**𓆰 تم انتحال حساب هاذ الفرخ😹🏃‍♀️. ༗**")
    if BOTLOG:
        await event.client.send_message(
            BOTLOG_CHATID,
            f"#الانتحـــال\n ⪼ تم انتحال حساب هاذ الفرخ😹🏃‍♀️. ↫ [{first_name}](tg://user?id={user_id }) بنجاح ✅",
        )
